...
public interface Expr {
	...
	class Unary implements Expr {
		...
		public float eval() {
			switch (this.op) {
				case UNARY_MINUS: return -this.arg.eval();
				case UNARY_BITWISE_NOT: return ~((int) this.arg.eval());
				case UNARY_LOGICAL_NOT: return ((int) this.arg.eval()) == 0 ? 1 : 0;
			}
			return 0;
		}
	}
			...
			for (String token : tokens) {
				...
				if (token.equals("(")) {
					...
					while (OPS.containsKey(o2) && ((Op.isLeftAssoc(op) &&
									op.ordinal() > OPS.get(o2).ordinal()) || (op.ordinal() > OPS.get(o2).ordinal()))) {
						Expr e = bind(o2, es);
						if (e == null) {
							return null;
						}
